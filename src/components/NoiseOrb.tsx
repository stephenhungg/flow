  ...
  useEffect(() => {
    ...
    class Molecule extends THREE.Object3D {
      ...
      dot(size = 32) {
        ...
        // Blue gradient for orb particles
        const gradient = ctx.createRadialGradient(sizeH, sizeH, 0, sizeH, sizeH, sizeH);
        gradient.addColorStop(0, '#4080C0');
        gradient.addColorStop(0.5, '#3060A0');
        gradient.addColorStop(1, '#204080');
...
float snoise(vec3 v) {
  ...
  return 105.0 * dot(m*m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3)));
        ...
        `
          ...
          (material as THREE.PointsMaterial & { userData: { shader: typeof shader } }).userData.shader = shader;
        };
      }

      animate(time: number) {
        this.mesh.rotation.set(0, time * 0.2, 0);
        const mat = this.material as THREE.PointsMaterial & { userData: { shader?: { uniforms: { time: { value: number } } } } };
